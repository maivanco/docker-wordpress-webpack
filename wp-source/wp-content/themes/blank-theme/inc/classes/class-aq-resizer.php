...
if(!function_exists('aq_resize')) {
    ...
    function render_cropped_image($params = []){
        ...
        $hard_crop = $params['width'] != 'auto' && $params['height'] != 'auto' ? true : false;

        //Define max-width & max-height if dev uses soft-crop
        $params['width'] = $params['width'] == 'auto' ? 1920 : $params['width'];
        $params['height'] = $params['height'] == 'auto' ? 1920 : $params['height'];

        $cropped_image_url =  aq_resize($params['url'], $params['width'], $params['height'], $hard_crop, true, true);


        $classes = is_array($params['class']) && !empty($params['class']) ? implode(' ', $params['class']) : $params['class'];
        if($params['return_only_url']){
            return $cropped_image_url;
        }
        echo '<img id="'.$params['id'].'" name="'.$params['name'].'" src="'.$cropped_image_url.'" alt="'.strip_tags($params['alt']).'" class="'.$classes.'" data-images="'.$params['data-images'].'">';
